#https://github.com/youngsoul/mediapipe-ymca/blob/master/uielements/uielements.py
from abc import ABC, abstractmethod # Abstract Base Class
from enum import Enum
import uuid # generate a unique identifier for each instance of the Shape class

import cv2 # OpenCV library for creating the UI
import imutils # convenience functions to make basic image processing operations such as translation, rotation, resizing, skeletonization, and displaying Matplotlib images easier with OpenCV.


# def read_image(image_path, width, height, mask_transparent):
#     """
#     Read an image from the given path and resize it to the specified width and height 
#     and handles transparent images by replacing transparent parts with white.

#     Args:
#         image_path (str): The path to the image file.
#         width (int): The width to resize the image to.
#         height (int): The height to resize the image to.
#         mask_transparent (bool): Whether to mask transparent parts of the image.
...
class Shape(ABC):
    ...
    """
    class Meta:
        # Nested class acting as a namespace for metadata associated with the Shape class.
        class State(Enum):
            INACTIVE = 0
            ENTER = 1
            EXIT = 2
...
class DisplayValueLabel(Shape):
    def __init__(self, x, y, width, height, label, bkgnd_color=(255, 182, 193), value_color=(255, 255, 255),
